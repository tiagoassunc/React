...
/*
 * High Order Component: insert animation logic on a component
 */
const withClapAnimation = (WrappedComponent) => {
  class WithClapAnimation extends Component {
    ...
    /* animate = () => {
      ...
      console.log("%c Debug ", "background: red;color:white");
    }; */
...
const MediumClap = ({ animationTimeline }) => {
  ...
  const handleClapClick = () => {
    ...
    setClapState((prevState) => ({
      isClicked: true,
      count: Math.min(prevState.count + 1, MAXIMUM_USER_CLAP), // Math.min() return the min
      countTotal:
  ...
  return (
    <span>
      <svg
        ...
        className={`${styles.icon} ${isClicked && styles.checked}`} // isClicked is true add styles.checked
      >
